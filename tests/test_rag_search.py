...
TEST_CASES = [
    ...
    {"page": 6, "q": "What is the annual ISA allowance?", "expect": ["20,000", "tax year"]},
    ...
    {"page": 8, "q": "Can I overpay my mortgage without a penalty?", "expect": ["10%", "erc"]},
    ...
    {"page": 9, "q": "Does the bank sell my personal data to advertisers?", "expect": ["no", "gdpr"]},
    {"page": 10, "q": "What happens to an account when someone passes away?", "expect": ["freeze", "death certificate"]},
    {"page": 10, "q": "Why would a bank close my account?", "expect": ["free", "discriminate"]},
    ...
    # --- Mortgages ---
    {"page": 10, "q": "What is the interest rate for the Fixed Rate Mortgage?", "expect": ["3.99%"]},
    {"page": 10, "q": "What is the minimum deposit required for a Fixed Rate Mortgage?", "expect": ["5%"]},
    {"page": 10, "q": "What is the minimum income requirement for the Fixed Rate Mortgage?", "expect": ["25,000"]},
    {"page": 10, "q": "How is the interest rate calculated on the Tracker Mortgage?", "expect": ["base rate + 1.5%"]},
    {"page": 11, "q": "What is the minimum deposit for the Tracker Mortgage?", "expect": ["10%"]},
    {"page": 12, "q": "What is the difference in the minimum deposit required for your mortgages?", "expect": ["fixed", "tracker"]},

    # --- Savings Accounts ---
    {"page": 11, "q": "What is the interest rate on the Instant Access Savings account?", "expect": ["4.25%"]},
    {"page": 11, "q": "Are there penalties for withdrawing money from the Instant Access Savings account?", "expect": ["no", "fixed term"]},
    {"page": 11, "q": "What is the minimum balance required for the Instant Access Savings account?", "expect": ["1"]},
    {"page": 14, "q": "What exactly is the penalty if I urgently need to withdraw money from my Fixed Rate Bond?", "expect": ["heavy penalty", "accrued interest"]},
    {"page": 15, "q": "What happens if I drop below the minimum balance on my savings accounts?", "expect": ["no interest", "closed"]},

    # --- Credit Cards & Loans ---
    {"page": 11, "q": "How much cashback can I earn with the Cashback Credit Card?", "expect": ["1%"]},
    {"page": 11, "q": "What is the interest rate on the Cashback Credit Card?", "expect": ["9.90%"]},
    {"page": 11, "q": "What is the interest rate for the Balance Transfer Card?", "expect": ["0%", "24 months"]},
    {"page": 11, "q": "Is there a fee to transfer my balance?", "expect": ["3%"]},
    {"page": 12, "q": "What credit score do I need for the Balance Transfer Card?", "expect": ["700"]},
    {"page": 12, "q": "What is the interest rate for a Personal Loan?", "expect": ["6.90%"]},
    {"page": 12, "q": "How much money can I borrow with a Personal Loan?", "expect": ["1,000", "35,000"]},
    {"page": 13, "q": "Which credit card is best for paying off existing debt?", "expect": ["balance transfer"]},

    # --- Penalties & Edge Cases ---
    {"page": 14, "q": "Is there an early settlement penalty for paying off my Personal Loan early?", "expect": ["58 days"]},
    {"page": 14, "q": "What is the penalty for missing a payment on the Cashback Credit Card?", "expect": ["12", "late"]},
    {"page": 14, "q": "What is the penalty if I don't pay off my Balance Transfer Card within 24 months?", "expect": ["standard variable", "apr"]},
    {"page": 14, "q": "Is there a penalty for withdrawing cash using my Cashback Credit Card?", "expect": ["cash advance fee", "daily"]},
    {"page": 15, "q": "Are there penalties for going overdrawn on my Current Account?", "expect": ["declined", "credit score"]},
]
...
@pytest.mark.asyncio
@pytest.mark.parametrize("test_case", TEST_CASES, ids=[f"Pg{t['page']}-{t['q'][:40]}..." for t in TEST_CASES])
